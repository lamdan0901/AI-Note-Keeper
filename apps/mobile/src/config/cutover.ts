...
const parseCutoverCohort = (value: string | undefined): CutoverCohort => {
  if (!value) {
    throw new Error('EXPO_PUBLIC_CUTOVER_COHORT is required');
  }
  ...
  throw new Error(`Unsupported EXPO_PUBLIC_CUTOVER_COHORT value: ${value}`);
};
...
export const readMobileCutoverConfig = (
  env: Readonly<Record<string, string | undefined>> = process.env,
): CutoverConfig => {
  return {
    cohort: parseCutoverCohort(env.EXPO_PUBLIC_CUTOVER_COHORT),
    requireParity: parseBooleanFlag(
      'EXPO_PUBLIC_CUTOVER_REQUIRE_PARITY',
      env.EXPO_PUBLIC_CUTOVER_REQUIRE_PARITY,
    ),
    requireSlo: parseBooleanFlag(
      'EXPO_PUBLIC_CUTOVER_REQUIRE_SLO',
      env.EXPO_PUBLIC_CUTOVER_REQUIRE_SLO,
    ),
    requireRollbackDrill: parseBooleanFlag(
      'EXPO_PUBLIC_CUTOVER_REQUIRE_ROLLBACK_DRILL',
      env.EXPO_PUBLIC_CUTOVER_REQUIRE_ROLLBACK_DRILL,
    ),
  };
};
...
export const evaluateMobileCohortTransition = (
