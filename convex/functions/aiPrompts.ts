...
export function buildParseVoiceNoteSystemPrompt(context: PromptContext): string {
  return [
    ...
    '{"draft":{"title":string|null,"content":string|null,"reminderAtEpochMs":number|null,"repeat":RepeatRule|null,"keepTranscriptInContent":boolean,"normalizedTranscript":string},"confidence":{"title":number,"content":number,"reminder":number,"repeat":number},"clarification":{"required":boolean,"question":string|null,"missingFields":["title"|"content"|"reminder"|"repeat"]}}',
    ...
    'If title/content confidence is low or content is empty, set keepTranscriptInContent=true.',
  ].join('\n');
}
